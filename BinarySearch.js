...
function binarySearch(sortedNums, searchNum) {
    let start = 0
    let end = sortedNums.length - 1 //last index
    let counter = 0
    while(start <= end){
        let mid = Math.floor((start+end)/2) //midpoint

        if (searchNum === sortedNums[mid]){
            counter++
                return true
            
        } //found target element in array
            else if (searchNum < sortedNums[mid]){ //search first half
                end = mid-1
            }
            else if (searchNum > sortedNums[mid]){ //search second half
                start = mid+1
            }
...
console.log(binarySearch(nums4, searchNum4)); // true (4 for bonus)
// bonus, how many times does the search num appear?
