...
function twoSum(nums, targetSum) {
    const numsVisited = {}; // {2: 0. 7:1} key array number, value = index
    const res = [];
    ...
    for (let i = 0 ; i < nums.length ; i++){
        ...
        console.log(numsVisited[complement])
        if (numsVisited[complement] !== undefined){ //exist already?
            res.push(numsVisited[complement]) //push the index of the complement in new arr
            res.push(i) //push current index
        }

        numsVisited[num] = i //if it doesnt exist in our object, add that key:value pair to our object where key = nums[i] value = i 
    }
    return res
}
