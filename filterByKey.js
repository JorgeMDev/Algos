...
const people = [
  ...
  function filterByKeyStartsWith(items, searchFor, searchBy) {
   ...
   // Filters the given items based on the search criteria using a startsWith
   let arr = []
    let charlenght = searchFor.length
   for (obj of items){

     for (const eachKey in obj){
    
        let objLow = obj[eachKey].toLowerCase()
        let searchLow = searchFor.toLowerCase()
         
       if (objLow.includes(searchLow) && eachKey == searchBy){

         let temp = objLow.slice(0,charlenght)
          console.log(temp)
          if (temp == searchLow){
