...
function isPalindrome(str) {
        for(var left=0; left<str.length/2; left++) {
            var right = str.length-1-left;  //saves the value to the right that need to be compared
            if(str[left] != str[right]) {   //compare first and last values of array, if are different then the array is not a palindrome
                return false;
            }
        }
        return true; //in case all are equal, the loop ends and return pal indrome
}
